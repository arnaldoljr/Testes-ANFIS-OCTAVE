%%With a suitable example demonstrate the perceptron learning law with 
%%its decision regions using MATLAB. 
%%Give the output in graphical form

%% Avaliar a evoluçao da aprendizagem da rede neural 
...
X = 2*rand(p-1, 2*N)-1; 
nn = round((2*N-1)*rand(N,1))+1; %% rotulos dos numeros randomicos 
X(:,nn) = sin(X(:,nn));
X = [X; ones(1,2*N)]; 
